...
var methods = {
    ...
    createTable: function (options, callback) {
        ...
        var self = this,
            params = {
                ...
                AttributeDefinitions: [
                    { AttributeName: self.hashKey, AttributeType: self.get(self.hashKey).key }
                ],
                KeySchema:[
        ...
        {
            params.AttributeDefinitions.push({ AttributeName: self.rangeKey, AttributeType: self.get(self.rangeKey).key });
            params.KeySchema.push({ AttributeName: self.rangeKey, KeyType: 'RANGE' });
        }
        ...
        if (self.hasSecondaryAttributes())
        {
            ...
            _.each(self.secondaryKeyAttributes, function(attr){
                params.AttributeDefinitions.push({ AttributeName: attr, AttributeType: self.get(attr).key });
        ...
        client.createTable(params, function(err){
    ...
    serializeKeyAttribute: function (val, attrs) {
        ...
        var value = _.map(attrs, function(attr, i){
            return self.get(attr).serialize(val[i]);
        }).join(self.keyDelimiter);

        //odedshr: I'm not sure these are the right values but original return ONLY string type...
        switch (typeof val[0]) {
    ...
    deserializeKeyAttribute: function (val, attrs) {
        ...
        if (val.S) {
            //odedshr: if val is a string, you might want to break, I'm not sure why christophercliff did this
            val = (attrs.length > 1) ? val.S.split(self.keyDelimiter) : [val.S];
        } else {
            //odedshr: however, if it's not a string, take the value of the first key (e.g. val={"N":1} => val=[1])
            //TODO: fix this ugly patch, deal with other kinds of values elegantly
            for (key in val) {
                val = [val[key]];
                break;
            }
