...
var Model = {
    ...
    findOne: function (attrs, callback) {
            ...
            && attrs[schema.hashKey])
        {
            ...
            return self.connection.client.getItem(params, function(err, data){
                if (err) return callback(err);
                callback(null, self.parse(data.Item), data.ConsumedCapacity.CapacityUnits);
            });
        }
    ...
    find: function (options, callback) {
        ...
        self.connection.client.query(query, function(err, data){
            //odedshr: err might contain statusCode=200 which means operation was successful, and then it's not really an error
            if (_.isNumber(err.statusCode) && err.statusCode!=200) return self.protectedCallback(callback,err);
            try
            {
            ...
            self.protectedCallback(callback,null, collection(_.map(data.Items, self.parse, self)), data.ConsumedCapacity.CapacityUnits);
        });
        
    },

    protectedCallback: function (callback,err,data, units) {
        //odedshr: self.connection.client.query is going to a crazy loop if there are errors thrown from the callback function
        //odedshr: I know it's rude to type unto somebody else's console, but it's rude to have bugs just the same..
        try{
            return callback(err,data, units);
        }
        catch (callbackError) {
